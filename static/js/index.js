...
function getEditEvaluationHTML(item) {
  ...
  let editedPrompt = item["Edited Prompt"];
  if (item["Edit"] == "Reweight") {
    editedPrompt += ` ('${item["Word"]}' intensified)`
  }
...
$.getJSON("static/data/entries.json", function (data) {
  ...
  surveyBuilder
    ...
    .addIntroduction(introductionBuilder =>
      introductionBuilder
        .withDescription(
          "This survey aims to evaluate the audio editing capabilities of artificial neural networks through two distinct sections: Naturalness and Faithfulness.",
          "In the Naturalness section, participants will encounter four pairs of 10-second audio clips. They will be asked to determine which clip sounds most natural to them, assessing the resemblance of these samples to real-world sounds.",
          "In the Faithfulness section, participants will encounter an additional 16 pairs of 10-second audio samples. Each pair will be accompanied by four questions designed to evaluate the extent to which the audio adheres to its provided description and preserves key elements such as melody, dynamics, and tempo. Participants will be tasked with rating the adherence to these specified characteristics, providing valuable insights into perceived audio quality and fidelity."
        )
        .addSeparator()
    ...
    .addDemographics(demographicsBuilder =>
      demographicsBuilder
        .withAgeQuestion("under 18", "18 to 29", "30 to 49", "50 to 69", "over 70")
        .withGenderQuestion("Male", "Female", "Other")
    ...
    .addSection(faithfulnessAudioPairs, (pageBuilder, item) =>
      pageBuilder
        ...
        .addRatingQuestion("How accurately does the edited audio preserve the melody of the original audio?")
        .addRatingQuestion("How faithfully does the edited audio preserve the intensity and dynamics of the original audio?")
        .addRatingQuestion("How faithfully does the edited audio maintain the tempo of the original audio?")
        .addRatingQuestion("How closely does the edited audio align with the edited description?")
    )
    .addFeedback();
